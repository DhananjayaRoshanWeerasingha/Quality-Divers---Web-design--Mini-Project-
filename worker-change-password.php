	...
	if(isset($_GET['user_id'])){
		...
		if($result_set){
			if(mysqli_num_rows($result_set) == 1){
				...
				//user not found
				header('Location:modify-worker.php? user_not_found');
				
			}
		}else{
			// query unsuccessful
			header('Location:modify-worker.php? err=query_failed');
		}
	}
	...
	if(isset($_POST['submit'])){
		...
		if(empty($errors)){
			...
			if($result){
				...
				header('Location:worker_details.php?user_modified=true');
...
<body>
 ...
 <br>
 <br>
 <form action="worker-change-password.php"  method="post" class="userform">
   ...
   <input type="text" name="last_name"<?php echo 'value="' . $last_name . '"'; //data form akata genima sadaha ?> disabled>
    ...
    <p>
   <lable for="">Email Address  &nbsp &nbsp: </lable>
   <input type="email" name="email"<?php echo 'value="' . $email . '"';  //data form akata genima sadaha?> disabled>
   </p>
   <p>
   <lable for="">Address &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp &nbsp: </lable>
   <input type="text" name="address"<?php echo 'value="' . $address . '"';  //data form akata genima sadaha?> disabled>
   </p>
   <p>
   <lable for="">Contact No. &nbsp &nbsp &nbsp &nbsp: </lable>
   <input type="text" name="phone_number"<?php echo 'value="' . $phone_number . '"';  //data form akata genima sadaha?> disabled>
   </p>
    <p>
   ...
   <a href="modify-worker.php?user_id=<?php echo$user_id; ?>" class="add"> Back </a>
